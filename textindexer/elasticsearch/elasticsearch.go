...
// type:keywords will be used for indexing
// type:text will be stored as blobs
var esMappings = `
{
  "mappings" : {
    "properties": {
...
var _ textindexer.Indexer = (*ElasticSearchIndexer)(nil)
...
func (i *ElasticSearchIndexer) Index(doc *textindexer.Document) error {
	if doc.LinkID == uuid.Nil {
		return xerrors.Errorf("index: %w", textindexer.ErrMissingLinkID)
	}
...
func (i *ElasticSearchIndexer) FindByID(linkID uuid.UUID) (*textindexer.Document, error) {
	...
	if len(searchRes.Hits.HitList) != 1 {
		return nil, xerrors.Errorf("find by ID: %w", textindexer.ErrNotFound)
	}
...
func (i *ElasticSearchIndexer) Search(q textindexer.Query) (textindexer.Iterator, error) {
	var qtype string
	switch q.Type {
	case textindexer.QueryTypePhrase:
		qtype = "phrase"
	default:
		qtype = "best_fields"
	}
...
func mapEsDoc(d *esDoc) *textindexer.Document {
	return &textindexer.Document{
...
func makeEsDoc(d *textindexer.Document) esDoc {
